...
/* These functions should not be called for Redox */
void _MD_YIELD(void) {
  PR_NOT_REACHED("_MD_YIELD should not be called for Redox.");
}

PRStatus _MD_CREATE_THREAD(PRThread* thread, void (*start)(void*),
                           PRThreadPriority priority, PRThreadScope scope,
                           PRThreadState state, PRUint32 stackSize) {
  PR_NOT_REACHED("_MD_CREATE_THREAD should not be called for Redox.");
  return PR_FAILURE;
}
#endif /* ! _PR_PTHREADS */
